...
`/create` uses the Node API to optimize the creative description before generation. The endpoint now supports Volcengine Ark DeepSeek first, with a local rule-based fallback.
...
- When `VOLCENGINE_ARK_API_KEY` or `ARK_API_KEY` is configured, the backend sends the original prompt and context to the Volcengine Ark DeepSeek endpoint ID `ep-20260528161712-9ncfp` (`DeepSeek-V4-pro`).
- DeepSeek is responsible for rewriting the prompt. The backend returns DeepSeek's final text directly as `rewrittenPrompt`.
- `duration`, `generationMode`, `ratio`, and `resolution` are passed as context only. The prompt rewrite instruction tells DeepSeek not to mechanically include ratio, resolution, or output-spec text unless the user asked for it.
- If the key is missing, the Ark request fails, times out, returns empty content, or returns an unexpected format, the endpoint returns the Chinese error `优化错误，请重试`.
...
DeepSeek integration lives in:

```text
apps/api/src/services/prompt-rewrite.service.ts
```

Keep supplier API keys in `apps/api/.env` only. Do not put DeepSeek or Volcengine Ark keys in frontend code or frontend environment files.
...
The sync script upserts providers by `providerKey` and models by `modelKey`. It does not delete database-only rows and does not store real API keys. Provider rows store only `apiKeyEnvName`, for example `VOLCENGINE_ARK_API_KEY`; the real value belongs in `.env`.
