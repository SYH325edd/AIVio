...
export type AdminUserDetail = {
  user: AdminUser;
  inviteSource: { code: string | null; inviter: { id: string; email: string; role: string } | null };
  orders: AdminOrder[];
  tasks: AdminTask[];
...
export type AdminGiftCard = {
  ...
  code: string;
  credits: number;
  status: "active" | "redeemed" | "disabled" | "expired" | string;
  expiresAt: string | null;
  createdByAdminId: string;
  redeemedByUserId: string | null;
  redeemedUser: { id: string; email: string; nickname: string } | null;
...
export function getAdminGiftCards(params?: { search?: string; status?: string; page?: number; pageSize?: number }) {
  const query = new URLSearchParams();
  Object.entries(params || {}).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); });
  const suffix = query.toString() ? `?${query.toString()}` : "";
  return apiRequest<{ items: AdminGiftCard[]; total: number; page: number; pageSize: number }>(`/admin/gift-cards${suffix}`);
}

export function createAdminGiftCards(payload: { credits: number; quantity: number; expiryType?: string; expiresAt?: string }) {
  return apiRequest<{ cards: AdminGiftCard[] }>("/admin/gift-cards", { method: "POST", body: JSON.stringify(payload) });
}

export function disableAdminGiftCard(id: string) {
  return apiRequest<AdminGiftCard>(`/admin/gift-cards/${id}/disable`, { method: "POST" });
}

export function deleteAdminGiftCard(id: string) {
  return apiRequest<{ id: string }>(`/admin/gift-cards/${id}`, { method: "DELETE" });
}

export function deleteAdminGiftCards(ids: string[]) {
  return apiRequest<{ deletedCount: number }>("/admin/gift-cards", { method: "DELETE", body: JSON.stringify({ ids }) });
}
