import { ClipboardList, Database, Grid2X2, PackageCheck, Plus, Users, WalletCards } from "lucide-react";
...
type ModuleKey = "overview" | "users" | "orders" | "tasks" | "credits" | "models" | "providers" | "logs";
...
const modelTemplates: ModelTemplate[] = [
  { key: "seedance-2-0", label: "Seedance 2.0", sortOrder: 10, rules: [pricingRule("480p", 8), pricingRule("720p", 18), pricingRule("1080p", 45)] },
  { key: "seedance-2-0-fast", label: "Seedance 2.0 Fast", sortOrder: 20, rules: [pricingRule("480p", 7), pricingRule("720p", 15)] },
  { key: "seedance-1-5-pro", label: "Seedance 1.5 Pro", sortOrder: 30, rules: [pricingRule("480p", 2, "silent"), pricingRule("720p", 4, "silent"), pricingRule("1080p", 7, "silent")] },
  { key: "seedance-1-0-pro-fast", label: "Seedance 1.0 Pro Fast", sortOrder: 40, rules: [pricingRule("480p", 1), pricingRule("720p", 2), pricingRule("1080p", 4)] }
];
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (activeModule === "orders") void loadOrders();
    if (activeModule === "tasks") void loadTasks();
    if (activeModule === "credits") void loadCreditLogs();
  ...
  async function savePricingRule(ruleId: string) {
    ...
    setMessage("");
    try {
      await updateAdminPricingRule(ruleId, draft);
      setMessage("计费规则已保存。");
  ...
  async function submitPricingRule(event: FormEvent) {
    ...
    setMessage("");
    try {
      await createAdminPricingRule(selectedPricingModelId, newPricingRule);
  ...
  return (
    <PageLayout className="admin-content">
      ...
      <section className="admin-module-tabs">
        {modules.map(([key, title]) => <button className={activeModule === key ? "active" : ""} type="button" key={key} onClick={() => setActiveModule(key)}>{title}</button>)}
      </section>
      {message ? <p className="auth-message success admin-message">{message}</p> : null}

      {activeModule === "overview" ? <Overview stats={stats} statCards={statCards} loading={loading.overview} error={errors.overview} /> : null}
      {activeModule === "users" ? <UsersModule users={users} total={totals.users} loading={loading.users} error={errors.users} savingId={savingId} adjustingUserId={adjustingUserId} adjustAmount={adjustAmount} adjustRemark={adjustRemark} setAdjustingUserId={setAdjustingUserId} setAdjustAmount={setAdjustAmount} setAdjustRemark={setAdjustRemark} saveAdjustment={saveAdjustment} saveUserStatus={saveUserStatus} /> : null}
      {activeModule === "orders" ? <OrdersModule orders={orders} total={totals.orders} loading={loading.orders} error={errors.orders} /> : null}
      {activeModule === "tasks" ? <TasksModule tasks={tasks} total={totals.tasks} loading={loading.tasks} error={errors.tasks} /> : null}
      {activeModule === "credits" ? <CreditsModule logs={creditLogs} total={totals.credits} loading={loading.credits} error={errors.credits} /> : null}
...
function Overview({ stats, statCards, loading, error }: { stats: AdminStats | null; statCards: StatItem[]; loading?: boolean; error?: string }) {
  return (
    <>
      {error ? <p className="data-error compact">{error}</p> : null}
      <section className="admin-stat-grid admin-stat-grid-wide">
        {statCards.map(([Icon, title, value]) => <StatCard key={title} icon={<Icon />} label={title} value={String(value)} />)}
      </section>
      {loading ? <Card className="admin-panel-state">数据总览加载中...</Card> : null}
      {!loading && !stats ? <Card className="admin-panel-state">暂无数据</Card> : null}
    </>
  );
}
    ...
    <>
      <AdminTable title="模型管理" loading={props.loading} error={props.error} empty={!props.models.length} action={<button className="admin-action primary" type="button" onClick={() => props.setCreating(!props.creating)}><Plus size={15} /> 新增模型</button>}>
        ...
        <DataTable rows={props.models} columns={[
          ...
          { key: "price", title: "旧单次价格", align: "right", render: (row) => <input className="admin-number-input" type="number" min="0" step="1" value={props.drafts[row.id]?.price ?? 0} onChange={(event) => props.setDrafts((current) => ({ ...current, [row.id]: { ...current[row.id], price: Number(event.target.value) } }))} /> },
          ...
          { key: "actions", title: "操作", render: (row) => <div className="admin-inline-actions"><button className="admin-action" type="button" disabled={props.savingId === row.id} onClick={() => props.saveModel(row.id)}>保存</button><button className="admin-action" type="button" disabled={props.savingId === row.id} onClick={() => props.toggleModel(row)}>{row.enabled ? "禁用" : "启用"}</button><button className="admin-action" type="button" onClick={() => props.setSelectedPricingModelId(row.id)}>计费规则</button><button className="admin-action danger" type="button" disabled={props.savingId === row.id} onClick={() => props.removeModel(row)}>删除</button></div> }
        ]} />
      </AdminTable>
      {props.selectedPricingModelId ? (
        <AdminTable title={`计费规则${selectedModel ? ` · ${selectedModel.displayName}` : ""}`} loading={props.pricingLoading} error={props.pricingError} empty={!props.pricingRules.length} action={<button className="admin-action primary" type="button" onClick={() => props.setCreatingPricingRule(!props.creatingPricingRule)}><Plus size={15} /> 新增计费规则</button>}>
          ...
          <PricingRulesTable rules={props.pricingRules} drafts={props.pricingDrafts} savingId={props.savingId} setDrafts={props.setPricingDrafts} save={props.savePricingRule} toggle={props.togglePricingRule} remove={props.removePricingRule} />
        </AdminTable>
      ) : null}
    </>
  );
...
function PricingRulesTable(props: {
  ...
  return <DataTable rows={props.rules} columns={[
    ...
    { key: "points", title: "每秒积分", align: "right", render: (row) => <input className="admin-number-input" type="number" min="1" step="1" value={props.drafts[row.id]?.pointsPerSecond ?? row.pointsPerSecond} onChange={(event) => patch(row.id, { pointsPerSecond: Number(event.target.value) })} /> },
    { key: "costPerSecondRmb", title: "火山估算成本", align: "right", render: (row) => <input className="admin-number-input" type="number" min="0" step="0.0001" value={props.drafts[row.id]?.costPerSecondRmb ?? ""} onChange={(event) => patch(row.id, { costPerSecondRmb: event.target.value ? Number(event.target.value) : null })} /> },
...
function PricingRuleForm({ draft, saving, setDraft, submit, cancel }: { draft: PricingDraft; saving: boolean; setDraft: Dispatch<SetStateAction<PricingDraft>>; submit: (event: FormEvent) => void; cancel: () => void }) {
  return (
    <form className="admin-editor" onSubmit={submit}>
      <div className="admin-editor-head"><strong>新增计费规则</strong><span>前台只展示积分消耗，后台规则不要填写真实密钥。</span></div>
      <div className="admin-form-grid">
        ...
        <label>每秒积分<input type="number" min="1" step="1" value={draft.pointsPerSecond} onChange={(event) => setDraft((current) => ({ ...current, pointsPerSecond: Number(event.target.value) }))} /></label>
        <label>火山估算每秒成本<input type="number" min="0" step="0.0001" value={draft.costPerSecondRmb ?? ""} onChange={(event) => setDraft((current) => ({ ...current, costPerSecondRmb: event.target.value ? Number(event.target.value) : null }))} /></label>
