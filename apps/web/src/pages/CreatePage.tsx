...
type ImageInputMode = "first" | "firstEnd";
...
const promptPlaceholder = "请输入你想生成的视频画面，例如：清晨的湖面、未来城市、产品展示、人物运动镜头……";
...
const modeOptions: Array<{ key: GenerationMode; label: string; description: string }> = [
  ...
  { key: "video", label: "视频参考生成", description: "根据参考视频控制运动" }
];
...
function modelSupports1080(model?: VideoModel | null) {
  ...
  return !model.id.includes("seedance-2-0-fast");
}
...
function generationModeLabel(mode: GenerationMode) {
  ...
  if (mode === "video") return "视频参考生成";
  return "文生视频";
}

function imageModeLabel(mode: ImageInputMode) {
  return mode === "firstEnd" ? "首尾帧生成" : "首帧生成";
}
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await createVideoGeneration({
        ...
        imageMode: generationMode === "image" ? (imageInputMode === "firstEnd" ? "first_last_frame" : "first_frame") : undefined,
        firstFrameAssetId: generationMode === "image" ? startImage?.asset?.id : undefined,
      ...
      <section className="create-workbench">
        <div className="create-preview-column">
          <Card className="preview-panel">
            <div className="card-title-row">
              ...
              <div className="tool-row preview-actions">
                <Button variant="secondary" icon={<RefreshCw size={14} />} type="button">重新生成</Button>
                <Button variant="secondary" icon={<RotateCcw size={14} />} type="button">重置</Button>
              </div>
            </div>
          ...
          <Card className="inspiration-card">
            <div className="card-title-row compact-row">
              <h3>创意灵感</h3>
              <button className="link-action" type="button"><RefreshCw size={13} /> 换一换</button>
            </div>
            <div className="inspiration-grid">
              {inspirations.map((item) => (
                <div className="inspiration" key={item.title}>
                  <span className={`thumb-large thumb-${item.image}`} />
                  <button type="button" aria-label={`使用${item.title}`} onClick={() => setPrompt(item.text)}>+</button>
        ...
        <Card className="create-control-column">
          <div className="control-form-stack">
            ...
            <div className="control-section material-summary-section">
              <button className="material-summary-card" type="button" onClick={() => setMaterialDrawerOpen((open) => !open)}>
                <div>
                  <span>素材</span>
              ...
              {materialDrawerOpen ? (
                <div className="inline-material-panel">
                  <div className="inline-material-head">
                    <strong>素材管理</strong>
                    <button type="button" onClick={() => setMaterialDrawerOpen(false)}>关闭</button>
                  </div>
                  <div className="inline-material-body">
                    {generationMode === "image" ? (
                      <div className="drawer-section">
                        <div className="submode-switch" role="tablist" aria-label="图片输入方式">
                          <button
                            className={imageInputMode === "first" ? "active" : ""}
                            type="button"
                            onClick={() => setImageInputMode("first")}
                          >
                            <FileImage size={14} /> 首帧生成
                          </button>
                          <button
                            className={imageInputMode === "firstEnd" ? "active" : ""}
                            type="button"
                            onClick={() => setImageInputMode("firstEnd")}
                          >
                            <FileImage size={14} /> 首尾帧生成
                          </button>
                        </div>
          ...
          <div className="create-action-sticky">
            ...
            <Button className="generate-btn" type="button" disabled={submitting || uploadingAssets || insufficient || !modelId} onClick={handleSubmit}>
