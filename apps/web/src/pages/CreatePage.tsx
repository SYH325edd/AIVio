...
function toPersistedMedia(value: UploadedImage | UploadedVideo | null): PersistedUploadedMedia | null {
...
function restoreImage(value: PersistedUploadedMedia | null | undefined): UploadedImage | null {
...
function restoreVideo(value: PersistedUploadedMedia | null | undefined): UploadedVideo | null {
  if (!value?.asset) return null;
  return {
    file: createPlaceholderFile(value.asset.originalName, value.asset.mimeType),
    previewUrl: value.previewUrl || value.asset.url,
...
function readCreatePageState(): CreatePageState | null {
...
function saveCreatePageState(state: CreatePageState) {
  if (typeof window === "undefined") return;
  try {
    window.sessionStorage.setItem(CREATE_PAGE_STATE_KEY, JSON.stringify(state));
  } catch {
    // Ignore storage errors so remembering inputs never blocks creation.
  }
}

export default function CreatePage() {
  const navigate = useNavigate();
  const initialStateRef = useRef<CreatePageState | null>(readCreatePageState());
  ...
  const [ratio, setRatio] = useState(() => initialState?.ratio || "16:9");
  const [generationMode, setGenerationMode] = useState<GenerationMode>(() => initialState?.generationMode || "text");
  const [imageInputMode, setImageInputMode] = useState<ImageInputMode>(() => initialState?.imageInputMode || "first");
  const [videoInputMode, setVideoInputMode] = useState<VideoInputMode>(() => initialState?.videoInputMode || "extension");
  const [outputDuration, setOutputDuration] = useState(() => initialState?.outputDuration || 0);
  const [inputVideoDuration, setInputVideoDuration] = useState(() => initialState?.inputVideoDuration || 4);
  const [resolution, setResolution] = useState(() => initialState?.resolution || "720p");
  const [audioMode, setAudioMode] = useState<"audio" | "silent" | "default">(() => initialState?.audioMode || "silent");
  const [motionStrength, setMotionStrength] = useState(() => initialState?.motionStrength || "medium");
  const [count, setCount] = useState(() => initialState?.count || 1);
  const [startImage, setStartImage] = useState<UploadedImage | null>(() => restoreImage(initialState?.startImage));
  const [endImage, setEndImage] = useState<UploadedImage | null>(() => restoreImage(initialState?.endImage));
  const [referenceFrames, setReferenceFrames] = useState<UploadedImage[]>(() => (initialState?.referenceFrames || []).map((item) => restoreImage(item)).filter(Boolean) as UploadedImage[]);
  const [referenceVideo, setReferenceVideo] = useState<UploadedVideo | null>(() => restoreVideo(initialState?.referenceVideo));
  ...
  const [advancedParamsOpen, setAdvancedParamsOpen] = useState(() => initialState?.advancedParamsOpen || false);
  ...
  useEffect(() => {
    saveCreatePageState({
      ...
      count,
      advancedParamsOpen,
      startImage: toPersistedMedia(startImage),
      endImage: toPersistedMedia(endImage),
      referenceFrames: referenceFrames
        .map((item) => toPersistedMedia(item))
        .filter(Boolean) as PersistedUploadedMedia[],
      referenceVideo: toPersistedMedia(referenceVideo)
    });
  }, [
    modelId,
    ratio,
  ...
  useEffect(() => () => {
    if (startImage?.previewUrl?.startsWith("blob:")) URL.revokeObjectURL(startImage.previewUrl);
  }, [startImage?.previewUrl]);

  useEffect(() => () => {
    if (endImage?.previewUrl?.startsWith("blob:")) URL.revokeObjectURL(endImage.previewUrl);
  }, [endImage?.previewUrl]);

  useEffect(() => () => {
    if (referenceVideo?.previewUrl?.startsWith("blob:")) URL.revokeObjectURL(referenceVideo.previewUrl);
