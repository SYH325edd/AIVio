...
function friendlyError(error: unknown) {
  ...
  if (error instanceof ApiError && error.status === 403) return "当前账号不可进行充值操作。";
    ...
    <PageLayout>
      <Card className="recharge-page-card">
        <div className="recharge-main">
          ...
          <p className="secure-line"><ShieldCheck size={18} /> 当前为 Mock 支付，仅用于本地测试，不会产生真实扣款。</p>
          {currentOrder ? (
            <Card className="order-current">
              <div className="card-title-row"><h3>当前订单</h3><StatusBadge status={statusType(currentOrder.status)}>{statusLabel(currentOrder.status)}</StatusBadge></div>
          ...
          <div className="pay-footer">
            ...
            {currentOrder?.status === "pending" ? (
              <Button type="button" disabled={paying} onClick={() => handleMockPay()}>{paying ? "支付中..." : "模拟支付成功"}</Button>
            ) : (
              <Button type="button" disabled={creating || !selectedPackageId} onClick={handleCreateOrder}>{creating ? "创建中..." : "立即充值"}</Button>
            )}
          </div>

          <Card className="orders-card">
            <div className="card-title-row"><h3>充值订单</h3><button className="mini-action" type="button" onClick={loadOrders}>刷新</button></div>
            <DataTable
              rows={orders}
              columns={[
                ...
                { key: "provider", title: "支付方式", render: (row) => row.paymentProvider },
                ...
                { key: "action", title: "操作", render: (row) => row.status === "pending" ? <button className="mini-action" type="button" disabled={paying} onClick={() => handleMockPay(row.id)}>模拟支付</button> : "--" }
              ]}
            />
            {!ordersLoading && orders.length === 0 ? <div className="empty-state">暂无充值订单</div> : null}
        ...
        <aside className="recharge-summary">
          ...
          <div className="benefit-list">
            ...
            <p><ShieldCheck /> <span><strong>Mock 支付</strong>仅用于本地测试，不产生真实扣款</span></p>
          </div>
