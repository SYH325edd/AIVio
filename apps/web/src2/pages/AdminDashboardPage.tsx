...
function calculateAdminSalesPrice(pointsPerSecond: number) {
  const salesCreditsPerSecond = Number.isFinite(pointsPerSecond) ? Math.max(0, Math.ceil(pointsPerSecond)) : 0;
  const salesCnyPerSecond = salesCreditsPerSecond / VIDEO_CREDIT_RATE;
  return { salesCnyPerSecond, salesCreditsPerSecond };
}

function pointsFromSalesCny(value: number) {
  if (!Number.isFinite(value) || value <= 0) return 1;
  return Math.ceil(value * VIDEO_CREDIT_RATE);
}
        ...
        {props.selectedPricingModelId ? (
          <div className="admin-editor">
            <div className="admin-editor-head">
              ...
              <span>成本元/秒为火山方舟估算成本；销售价格元/秒为对用户收费价格，实际扣费时按 1 元 = 100 积分换算。</span>
            </div>
...
function PricingRulesTable(props: {
  ...
  return <DataTable rows={props.rules} columns={[
    ...
    { key: "costPerSecondRmb", title: "成本元/秒", align: "right", render: (row) => <input className="admin-number-input" type="number" min="0" step="0.0001" value={props.drafts[row.id]?.costPerSecondRmb ?? ""} onChange={(event) => patch(row.id, { costPerSecondRmb: event.target.value ? Number(event.target.value) : null })} /> },
    { key: "salesCnyPerSecond", title: "销售价格元/秒", align: "right", render: (row) => {
      const sales = calculateAdminSalesPrice(props.drafts[row.id]?.pointsPerSecond ?? row.pointsPerSecond);
      return <input className="admin-number-input" type="number" min="0.01" step="0.01" value={formatAdminCny(sales.salesCnyPerSecond)} onChange={(event) => patch(row.id, { pointsPerSecond: pointsFromSalesCny(Number(event.target.value)) })} />;
    } },
...
function PricingRuleForm({ draft, saving, setDraft, submit, cancel }: { draft: PricingDraft; saving: boolean; setDraft: Dispatch<SetStateAction<PricingDraft>>; submit: (event: FormEvent) => void; cancel: () => void }) {
  const sales = calculateAdminSalesPrice(draft.pointsPerSecond);
  return (
    <form className="admin-editor" onSubmit={submit}>
      <div className="admin-editor-head"><strong>新增计费规则</strong><span>只填写成本和销售价格；实际扣费时按 1 元 = 100 积分换算。</span></div>
      <div className="admin-form-grid">
        ...
        <label>成本元/秒<input type="number" min="0" step="0.0001" value={draft.costPerSecondRmb ?? ""} onChange={(event) => setDraft((current) => ({ ...current, costPerSecondRmb: event.target.value ? Number(event.target.value) : null }))} /></label>
        <label>销售价格元/秒<input type="number" min="0.01" step="0.01" value={formatAdminCny(sales.salesCnyPerSecond)} onChange={(event) => setDraft((current) => ({ ...current, pointsPerSecond: pointsFromSalesCny(Number(event.target.value)) }))} /></label>
